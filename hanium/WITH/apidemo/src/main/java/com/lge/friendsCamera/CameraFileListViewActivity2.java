 ...
 */
public class CameraFileListViewActivity2 extends AppCompatActivity {
    private TimerTask mTask;
    private Timer mTimer;

    private Switch autoPicture;

    private final static String TAG = CameraFileListViewActivity2.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        autoPicture.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton cb,
                                         boolean isChecking) {
                if(isChecking){
                    mTimer.schedule(mTask,10000,15000);
                }
                else{
                    mTimer.cancel();
                }
...
//        Intent intent = getIntent();
    ...
    protected void onResume() {
        ...
        if (!FriendsCameraApplication.getIsConnected()) {
            ((CameraFileListViewActivity2) mContext).finish();
        }
    }
    ...
    private void getListFiles(String token) {
        ...
        if (parameters != null) {
            ...
            commandsExecute.setListener(new OnHttpListener() {
                @Override
    ...
    protected void getFullFile_(final int position) {
        currentDownloadFile = adapter.getInfo(position, OSCParameterNameMapper.FileInfo.NAME);
        JSONObject parameters = new JSONObject();
        try {
            String url = adapter.getInfo(position, OSCParameterNameMapper.FileInfo.URL);
    ...
    private void deleteFilesFromCamera(final int[] positions, final boolean removeAll) {
        ...
        cmdExecute.setListener(new OnHttpListener() {
            @Override
            public void onResponse(OSCReturnType type, Object response) {
